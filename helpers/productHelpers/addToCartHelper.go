...
func addToCart(userId primitive.ObjectID, newProduct map[string]interface{},count string) {
	...
	if len(cart) != 0 {
		for _, product := range cart["products"].(primitive.A) {
			if product.(map[string]interface{})["productId"].(primitive.ObjectID) == newProduct["productId"].(primitive.ObjectID) {
				if count == "" {
					var _, err = db.Collection("cart").UpdateOne(ctx, bson.M{"products.productId": newProduct["productId"].(primitive.ObjectID), "userId": userId}, bson.M{"$inc": bson.M{"products.$.quantity": 1}})
